...
def progress_bar(current, total, msg=''):
    ...
    message = f' [{percent:.2f}%] | {"#" * progress}{"-" * (50 - progress)} | {msg}'
    print(message, end='\r')
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        print("inputs and targets are assigned")
        inputs, targets = inputs.to(device), targets.to(device)

        optimizer.zero_grad()
        print("optimizer is cleared")
        
        outputs = net(inputs)
        #assert ((0 <= targets) & (targets < outputs.shape[-1])).all()
        #assert ((0 <= targets) ).all()
        #assert ( (targets < outputs.shape[-1])).all()
        


        print("Output shape:", outputs.shape)  # Add this line
        print("Target shape:", targets.shape)  # Add this line


        
        loss = criterion(outputs, targets)
        print("Loss calculated:", loss.item())  # Add this line

        loss.backward()
        optimizer.step()
        print("optimizer is stepped")

        train_loss += loss.item()
        print("train_loss is updated")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr', default=0.1, type=float, help='learning rate') ## chnaged learnign rate from 0.1 to 0.15
